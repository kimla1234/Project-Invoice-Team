...
const ProductModal = ({ isOpen, onClose, onSelectProducts }: ProductModalProps) => {
      ...
      <div className="relative z-50 max-h-[80vh] w-[600px] overflow-y-auto rounded-lg border bg-white p-4 shadow-lg">
        ...
        <div className="max-h-[300px] space-y-2 overflow-y-auto">
          {products.filter(p => p.name.toLowerCase().includes(searchTerm.toLowerCase())).map((p) => (
            ...
            >
              <span>{p.name}</span>
              <input type="checkbox" checked={selectedProducts.has(p.id)} readOnly />
            </div>
          ))}
...
export default function CreateQuotation() {
  ...
  useEffect(() => {
    ...
    const maxId = oldQuotations.length > 0 ? Math.max(...oldQuotations.map((q: any) => q.id)) : 0;
  ...
  const handleItemChange = (index: number, field: "name" | "qty" | "unitPrice", value: string | number) => {
    setItems((prev) =>
      prev.map((item, i) => {
        if (i !== index) return item;
        const updated: Item = { ...item };
        if (field === "qty" || field === "unitPrice") updated[field] = Number(value);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedClient) {
      toast({ title: "Select client", description: "Please select a client.", className: "bg-red-600 text-white", duration: 3000 });
      return;
    }
    if (items.length === 0) {
      toast({ title: "No items", description: "Please add at least one item.", className: "bg-red-600 text-white", duration: 3000 });
      return;
    }
    ...
    localStorage.setItem("quotations", JSON.stringify([...oldQuotations, newQuotation]));

    toast({ title: "Quotation Created", description: `${newQuotation.quotationNo} created successfully!`, className: "bg-green-600 text-white", duration: 3000 });
      ...
      <form onSubmit={handleSubmit} className="space-y-6 rounded-lg border bg-white p-6">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <div>
            ...
            <input type="text" value={quotationNo} readOnly className="w-full rounded border bg-gray-100 p-2" />
          </div>
          <div>
        ...
        <div className="rounded border p-4">
          <div className="mb-2 flex justify-between">
            ...
            <button type="button" onClick={() => setIsModalOpen(true)} className="rounded bg-primary px-3 py-1 text-white hover:bg-primary/90">Add Items</button>
          </div>
              ...
              {items.map((item, index) => (
                <tr key={index}>
                  ...
                  <td className="px-2 py-1">
                    <input className="w-full p-1" value={item.name} onChange={(e) => handleItemChange(index, "name", e.target.value)} />
                  </td>
                  <td className="px-2 py-1">
                    <input type="number" className="w-full p-1" value={item.qty} onChange={(e) => handleItemChange(index, "qty", e.target.value)} />
                  </td>
                  <td className="px-2 py-1">
                    <input type="number" className="w-full p-1" value={item.unitPrice} onChange={(e) => handleItemChange(index, "unitPrice", e.target.value)} />
                  </td>
                  <td className="px-2 py-1">{item.total.toFixed(2)}</td>
                  <td className="px-2 py-1">
                    <button type="button" className="rounded bg-red-500 px-2 py-1 text-white" onClick={() => removeItem(index)}>Delete</button>
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        <button type="submit" className="w-full rounded bg-primary py-3 text-white hover:bg-primary/90">Create Quotation</button>
      </form>

      <ProductModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} onSelectProducts={handleAddProducts} />
    </div>
  );
}
