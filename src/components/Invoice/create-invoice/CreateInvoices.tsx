...
const ProductModal = ({ isOpen, onClose, onSelectProducts }: ProductModalProps) => {
  ...
  useEffect(() => {
    ...
    async function fetchProducts() {
      const data = await import("@/components/Tables/fetch").then((mod) => mod.getProductsTableData());
      setProducts(data);
    }
    fetchProducts();
  }, [isOpen]);
...
export default function CreateInvoice() {
  ...
  useEffect(() => {
    import("@/components/Tables/clients").then((mod) => setClients(mod.mockClients));
  }, []);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    toast({ title: "Invoice Created", description: `${newInvoice.invoiceNo} created successfully!`, className: "bg-green-600 text-white", duration: 3000 });
  ...
  return (
    <div className="p-10">
      <form onSubmit={handleSubmit} className="space-y-6 rounded-lg border bg-white p-6">
        ...
        <button type="submit" className="w-full rounded bg-primary py-3 text-white hover:bg-primary/90">Create Invoice</button>
      </form>
